...
class ProductsController extends Controller
{
    ...
    public function shop(Request $request)
    {
        ...
        if (!empty($request->category) and $request->category != 'all') {
            ...
            if(!empty($category) and count($category)>0){
                ...
                if ($category[0]->parent_id == 0) {
                    ...
                    $child_categories = $this->products->getChildCategories($category[0]->categories_id);
        ...
        $result['child_count'] = $child_count = count($child_categories);
        if(!empty($child_categories) and $child_count>0){
            $result['child_present'] = 1;
        }
        else{
            $result['child_present'] = 0;
        }
